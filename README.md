...
*java.util.concurrent.CountDownLatch*. The server waiths for HTTP requests on a designated port (the seccond cmd argument when running the jar), processes them and sends responses back to the clients. The application only sends and uploads static resources from and to specified directories (*downloads* and *uploads*) located in the root folder of the application (where the jar is located).
